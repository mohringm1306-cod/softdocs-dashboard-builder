	...
	return fakeNames.slice(0, count).map(function (name, i) {
		...
		columns.forEach(function (col) {
			...
			if (
				...
				colLower.includes("requester") ||
				colLower.includes("student") ||
				colLower.includes("employee")
			) {
				row[col] = name;
			} else if (
				colLower.includes("date") ||
				colLower.includes("submitted") ||
				colLower.includes("created")
			) {
				row[col] = fakeDates[i];
			} else if (
				colLower.includes("status") ||
				colLower.includes("step") ||
				colLower.includes("workflow")
			) {
				row[col] = fakeStatuses[i];
			} else if (
				colLower.includes("dept") ||
				colLower.includes("department") ||
				colLower.includes("office")
			) {
				row[col] = fakeDepts[i];
...
function getPreviewColumns() {
	...
	if (State.mode === "content") {
		var fields =
			SimulatedData.keyFields[(State.selectedArea || {}).id] || [];
		columns = fields
			.filter(function (f) {
		...
		var inputs =
			SimulatedData.formInputIds[(State.selectedTemplate || {}).id] || [];
		columns = inputs
			.filter(function (i) {
	...
	var tbHtml = data
		.map(function (row) {
			var cells =
				...
				(showView
					? '<td><span class="preview-view-btn"><i class="bi bi-eye"></i> View</span></td>'
					: "");
			return "<tr>" + cells + "</tr>";
		})
		.join("");
...
function previewSwimlane(name, idx, contentHtml, count, filterSummary) {
	...
	return (
		...
		slId +
		"');var ic=this.querySelector('i');if(c.style.display==='none'){c.style.display='';ic.className='bi bi-chevron-down';}else{c.style.display='none';ic.className='bi bi-chevron-right';}\" style=\"cursor:pointer;\">" +
...
function previewFilterSummary(sl) {
	...
	return sl.filters
		.map(function (f) {
			return (
				escapeHtml(f.fieldName || f.field) +
				": " +
				f.values
					.slice(0, 2)
					.map(function (v) {
...
function previewHeader(title, subtitle, styleDef) {
	var styleTag = styleDef
		? '<div style="display:flex;align-items:center;gap:6px;padding:6px 10px;background:#f0f7f4;border-radius:6px;margin-bottom:8px;font-size:0.7rem;color:var(--primary);">' +
			'<i class="bi ' +
			escapeHtml(styleDef.icon) +
			'"></i> ' +
			escapeHtml(styleDef.name) +
			"</div>"
		: "";
	return (
		'<div class="preview-label"><i class="bi bi-eye"></i> Live Preview</div>' +
		'<div class="preview-dashboard">' +
		'<div class="preview-dashboard-header">' +
		'<h4><i class="bi bi-grid-3x3-gap"></i> ' +
		escapeHtml(title) +
		"</h4>" +
		"<small>" +
		escapeHtml(subtitle) +
...
function renderPreview() {
	...
	if (State.colors) {
		container.style.setProperty(
			"--primary",
			State.colors.primary || "#006341",
		);
		container.style.setProperty(
			"--primary-dark",
			State.colors.primaryDark || "#004d35",
		);
		container.style.setProperty(
			"--accent",
			State.colors.accent || "#f4b41a",
		);
	}
	...
	switch (style) {
		...
		case "simple-status":
			...
			break;
		case "request-type":
			bodyHtml = previewRequestType(columns, swimlanes);
			break;
		case "alpha-split":
			bodyHtml = previewAlphaSplit(columns, swimlanes);
			break;
		// Advanced styles (wizard-preview-advanced.js)
		case "expandable":
			bodyHtml = previewExpandable(columns, swimlanes);
			break;
		case "claims":
			bodyHtml = previewClaims(columns, swimlanes);
			break;
		case "workflow-actions":
			bodyHtml = previewWorkflowActions(columns, swimlanes);
			break;
		case "pdf-signatures":
			bodyHtml = previewPdfSignatures(columns, swimlanes);
			break;
		case "bulk-actions":
			bodyHtml = previewBulkActions(columns, swimlanes);
			break;
		// Specialized styles (wizard-preview-specialized.js)
		case "survey-analytics":
			bodyHtml = previewSurveyAnalytics(columns, swimlanes);
			break;
		case "committee-voting":
			bodyHtml = previewCommitteeVoting(columns, swimlanes);
			break;
		case "cards-dashboard":
			bodyHtml = previewCardsDashboard(columns, swimlanes);
			break;
		case "award-nominations":
			bodyHtml = previewAwardNominations(columns, swimlanes);
			break;
		default:
			bodyHtml = previewSimpleStatus(columns, swimlanes);
	}
	...
	container.innerHTML =
		previewHeader(title, subtitle, styleDef) + bodyHtml + "</div>";
}
