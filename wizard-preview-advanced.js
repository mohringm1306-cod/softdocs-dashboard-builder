...
function previewExpandable(columns, swimlanes) {
	...
	if (detailNames.length === 0)
		detailNames = ["Budget Amount", "Department", "Notes"];
	...
	swimlanes.forEach(function (sl, idx) {
		...
		var tbHtml = data
			.map(function (row, rIdx) {
				...
				var rowHtml =
					"<tr" +
					(showDetail ? ' style="background:#f0f7f4;"' : "") +
					">" +
				...
				if (showDetail) {
					rowHtml +=
						...
						(displayCols.length + 2) +
						'" style="background:#f8fdf9;padding:6px 10px 6px 30px;border-left:3px solid var(--primary);">' +
						'<div style="display:grid;grid-template-columns:repeat(' +
						Math.min(detailNames.length, 3) +
						',1fr);gap:6px;font-size:0.6rem;">' +
						detailNames
							.map(function (name) {
								return (
									'<div><strong style="color:#666;">' +
									escapeHtml(name) +
									':</strong> <span style="color:#333;">Sample data</span></div>'
								);
							})
							.join("") +
						"</div></td></tr>";
...
function previewClaims(columns, swimlanes) {
	...
	var chips = State.styleConfig.filterChips || [
		"All",
		"High Priority",
		"30+ Days",
		"60+ Days",
	...
	// Personal stats bar
	var html =
		'<div style="background:#f8f9fa;border-left:3px solid var(--primary);padding:8px 12px;margin-bottom:8px;border-radius:4px;font-size:0.65rem;">' +
	...
	chips.slice(0, 5).forEach(function (c, i) {
		html +=
			...
			(i === 0 ? "var(--primary)" : "#e9ecef") +
			";color:" +
			(i === 0 ? "#fff" : "#333") +
			';border-radius:12px;font-size:0.6rem;cursor:pointer;">' +
			escapeHtml(c) +
			"</span>";
	});
	html += "</div>";
	...
	swimlanes.forEach(function (sl, idx) {
		...
		var tbHtml = data
			.map(function (row, rIdx) {
				...
				return (
					"<tr" +
					(claimed ? ' style="background:#e8f5e9;"' : "") +
					">" +
					displayCols
						.map(function (col) {
							return (
...
function previewWorkflowActions(columns, swimlanes) {
	...
	swimlanes.forEach(function (sl, idx) {
		...
		var rawActions = (State.styleConfig.workflowActions || {})[sl.name] || [
			"Approve",
			"Deny",
		];
		// Normalize: actions can be strings ('Approve') or objects ({label, icon, btnStyle})
		var actions = rawActions.map(function (a) {
			return typeof a === "string" ? a : a.label || "Action";
		});
		...
		var tbHtml = data
			.map(function (row) {
				var actionBtns = actions
					.slice(0, 3)
					.map(function (a) {
						var isApprove = a.toLowerCase().includes("approve");
						var isDeny = a.toLowerCase().includes("deny");
						var btnColor = isApprove
							? "#28a745"
							: isDeny
								? "#dc3545"
								: "#6c757d";
						return (
							'<span style="padding:1px 5px;background:' +
							btnColor +
							';color:#fff;border-radius:3px;font-size:0.5rem;cursor:pointer;margin-right:2px;">' +
							escapeHtml(a) +
							"</span>"
						);
					})
		...
		html +=
			...
			'<div class="preview-swimlane-header" onclick="var c=document.getElementById(\'' +
			slId +
			"');var ic=this.querySelector('i');if(c.style.display==='none'){c.style.display='';ic.className='bi bi-chevron-down';}else{c.style.display='none';ic.className='bi bi-chevron-right';}\" style=\"cursor:pointer;border-left:3px solid " +
			color +
			';">' +
			...
			"</div>" +
			(previewFilterSummary(sl)
				? '<div class="preview-filter-hint"><i class="bi bi-funnel"></i> ' +
					previewFilterSummary(sl) +
					"</div>"
				: "") +
			'<div id="' +
			slId +
			'" class="preview-swimlane-body">' +
			tableHtml +
			"</div>" +
			"</div>";
	});
...
function previewPdfSignatures(columns, swimlanes) {
	...
	var detailNames = detailFields
		...
		.slice(0, 3);
	if (detailNames.length === 0)
		detailNames = ["Signature 1", "Signature 2", "Completion Date"];
...
function previewBulkActions(columns, swimlanes) {
	...
	// Action bar
	var html =
		'<div style="background:#f8f9fa;padding:6px 10px;border-radius:6px;margin-bottom:8px;display:flex;gap:4px;align-items:center;flex-wrap:wrap;">' +
	...
	swimlanes.forEach(function (sl, idx) {
		...
		var tbHtml = data
			.map(function (row, rIdx) {
				...
				return (
					"<tr" +
					(checked ? ' style="background:#e3f2fd;"' : "") +
					">" +
...
if (typeof define === "function" && define.amd) {
	define("template/wizard-preview-advanced", [
