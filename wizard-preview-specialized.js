...
function previewSurveyAnalytics(columns, swimlanes) {
		...
		.map(function (row, i) {
			...
			return (
				...
				displayCols
					...
					.join("") +
				'<td><span style="padding:1px 6px;background:' +
				color +
				"20;color:" +
				color +
	...
	html +=
		'<div style="margin-top:8px;font-size:0.55rem;color:#888;"><i class="bi bi-info-circle"></i> Cards view shows rating badges and comment excerpts</div>';
	html +=
		'<div style="display:grid;grid-template-columns:1fr 1fr;gap:4px;margin-top:4px;">';
	ratings.slice(0, 2).forEach(function (r, i) {
		var color = r >= 4 ? "#28a745" : r >= 3 ? "#ffc107" : "#dc3545";
		html +=
			'<div style="border:1px solid #e9ecef;border-left:3px solid ' +
			color +
			';border-radius:4px;padding:6px;font-size:0.55rem;">' +
			...
			escapeHtml(data[i][displayCols[displayCols.length - 1]] || "Dept") +
			"</small>" +
			"</div>" +
			'<div style="color:#555;">' +
			comments[i] +
...
function previewCommitteeVoting(columns, swimlanes) {
	...
	// Logged-in-as bar
	var html =
		'<div style="background:var(--primary);color:#fff;padding:6px 10px;border-radius:4px;margin-bottom:8px;display:flex;justify-content:space-between;align-items:center;font-size:0.65rem;">' +
	...
	var votePatterns = [
		{
			votes: ["approved", "approved", "pending", "", ""],
			result: "2/3 approved",
		},
		{
			votes: ["approved", "denied", "approved", "", ""],
			result: "2/3 approved",
		},
		{
			votes: ["pending", "pending", "pending", "", ""],
			result: "No votes yet",
		},
		{
			votes: ["denied", "approved", "more-info", "", ""],
			result: "Split decision",
		},
	];

	// Member headers
	var memberTh = members
		.map(function (m) {
			return (
				'<th style="background:' +
				escapeHtml(m.color) +
				';font-size:0.55rem;padding:2px 4px;text-align:center;min-width:40px;">' +
				escapeHtml(m.name.split(" ").pop()) +
				"</th>"
			);
		})
		.join("");
						...
						vote === "approved"
							...
							: vote === "denied"
								...
								: vote === "more-info"
									...
									: vote === "pending"
										? '<i class="bi bi-clock" style="color:#aaa;"></i>'
...
function previewCardsDashboard(columns, swimlanes) {
	...
	html +=
		'<div style="flex:2;background:#fff;border:1px solid #e9ecef;border-radius:6px;padding:10px;">' +
	...
	Object.keys(counts).forEach(function (status) {
		...
		html +=
			'<div style="display:flex;align-items:center;gap:6px;margin-bottom:3px;font-size:0.6rem;">' +
	...
	// Total card
	html +=
		'<div style="flex:1;background:#fff;border:1px solid #e9ecef;border-radius:6px;padding:10px;display:flex;flex-direction:column;align-items:center;justify-content:center;">' +
	...
	statuses.forEach(function (status, i) {
		var color = statusColors[status] || "#888";
		html +=
			'<div style="background:#fff;border:1px solid #e9ecef;border-radius:6px;padding:8px;font-size:0.6rem;">' +
			'<div style="display:flex;justify-content:space-between;align-items:start;margin-bottom:4px;">' +
			'<div><div style="font-weight:600;color:#333;font-size:0.65rem;">' +
			escapeHtml(titles[i]) +
			"</div>" +
			'<div style="color:#888;font-size:0.55rem;">Lead: ' +
			escapeHtml(leads[i]) +
			"</div></div>" +
			'<span style="padding:1px 6px;background:' +
			color +
			"15;color:" +
			color +
			";border:1px solid " +
			color +
			'30;border-radius:8px;font-size:0.5rem;font-weight:600;white-space:nowrap;">' +
			escapeHtml(status) +
			"</span>" +
			"</div>" +
			(budgetField
				? '<div style="color:#888;font-size:0.55rem;">Budget: ' +
					budgets[i] +
					"</div>"
				: "") +
	...
	swimlanes.forEach(function (sl, idx) {
		...
		var tbHtml = data
			.map(function (row, rIdx) {
				...
				var rowHtml =
					"<tr" +
					(showDetail ? ' style="background:#fffaf0;"' : "") +
					">" +
				...
				if (showDetail) {
					rowHtml +=
						...
						'" style="background:#fffdf5;padding:8px 10px 8px 30px;border-left:3px solid ' +
						catColor +
						';">' +
						'<div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;font-size:0.6rem;">' +
						'<div><strong style="color:#666;"><i class="bi bi-trophy" style="color:' +
						catColor +
						';"></i> Nominee:</strong> ' +
						escapeHtml(nominees[rIdx]) +
						"</div>" +
						'<div><strong style="color:#666;"><i class="bi bi-person"></i> Nominated by:</strong> ' +
						escapeHtml(nominators[rIdx]) +
						"</div>" +
		...
		var tableHtml =
			...
			tbHtml +
			"</tbody></table>";
		html += previewSwimlane(
			sl.name,
			idx,
			tableHtml,
			rowCount,
			previewFilterSummary(sl),
		);
	});
...
if (typeof define === "function" && define.amd) {
	define("template/wizard-preview-specialized", [
