...
function previewRequestType(columns, swimlanes) {
	...
	swimlanes.forEach(function (sl, idx) {
		...
		html +=
			...
			";border:1px solid " +
			color +
			'30;border-radius:12px;font-size:0.6rem;font-weight:600;">' +
			escapeHtml(sl.name) +
			" (" +
			count +
			")</span>";
...
function previewAlphaSplit(columns, swimlanes) {
	...
	ranges.forEach(function (r) {
		var label = r[0] + "-" + r[1];
		html +=
			'<span style="padding:2px 8px;background:var(--primary);color:#fff;border-radius:12px;font-size:0.6rem;font-weight:600;">' +
			escapeHtml(label) +
			"</span>";
	});
	html += "</div>";
	...
	ranges.forEach(function (r, idx) {
		var label = r[0] + "-" + r[1];
		var names = namesByRange[label] || [
			"Student " + (idx + 1) + "A",
			"Student " + (idx + 1) + "B",
		];
		var data = generateFakePreviewData(
			displayCols,
			Math.min(names.length, 3),
		);
...
if (typeof define === "function" && define.amd) {
	define("template/wizard-preview-basic", [
