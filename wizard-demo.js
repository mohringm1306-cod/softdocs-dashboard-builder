...
// ============================================================================
var WIZARD_VERSION = '4.7.0';
var WIZARD_BUILD_DATE = '2026-08-18';

// Changelog (newest first)
// 4.7.0 (2026-08-18) - Generated dashboards no longer offer a View link that cannot work. Central resolves an in-flight item through /flow/api/user-dashboard/packages?taskId=, which is scoped to the signed-in user: it answers null for anyone but the assignee, and null leaves the tab spinning with no error. View is now built in ONE place for every style (_viewLink), renders disabled with the reason when the row sits in another person's queue, and probes that endpoint on click so a null answer closes the blank tab and says why. Group queues and dashboards built without the Assignee column keep the old behavior. Also: a Diagnostics strip in the page collects the failures that used to reach only console.error (data-source errors, filter fields that match no column, blocked View links) with a Copy for support button, so the dashboard owner can paste facts into a ticket without opening developer tools.
