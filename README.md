...
- **Fixed: View could not work at all on a row sitting in someone else's queue, and said nothing.** This is the last of four separate causes behind "View just spins," and the only one that was never a bug in the query. Central resolves an in-flight `taskId` link through `GET /flow/api/user-dashboard/packages?taskId=`, and that endpoint is scoped to the signed-in user: it returns the package when the task is the caller's, and `null` for everyone else. `null` leaves the tab on a blank page with a spinner and no error text. On a departmental dashboard, where the whole point is that you can see the whole department's work, that means every in-flight row looks broken to everybody except the one person it is assigned to. Nothing was wrong with the data, so no amount of checking the query would have found it.
  ...
  The generated dashboard now stops offering a link that cannot work. Every style builds its View control in one place, and when a row is in another person's queue it renders greyed out and says which queue, instead of a green button that opens a spinner. Clicking any in-flight View also asks that same endpoint first, so if the answer is `null` the blank tab closes and the dashboard says why. Two things are deliberately left alone: a row assigned to a **group** stays clickable, because you may well be in that group, and a dashboard built **without** the Assignee column behaves exactly as it did before. Neither case can be judged up front, and the click check catches both.

- **New: a Diagnostics panel in the dashboard itself.** Failures used to go only to the browser console, which the person who owns a dashboard does not open, so problems reached support as "it just spins" and nothing more. The dashboard now collects what went wrong on the page (a source that would not load, a swimlane filter that matches no column, a View link Central refused) with timestamps, and shows a **Copy for support** button. Paste that into your ticket and the answer is usually immediate. The panel stays hidden until there is something to report.
